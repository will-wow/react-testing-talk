...
Finally there's react test renderer, which takes the opposite approach, with a low-level api.
...
So more like enzyme, but even lower level. A totally
...
Enyzme and RTL are whole websites, API of RTR is
one page
...
- Enzyme Mount for multi-component components (like a date picker)
...
Did a mount snapshot for every page in the app, so everything
got covered by some test. They were sort of a pain
...
In the end, the testing technique for all these tools is the same
