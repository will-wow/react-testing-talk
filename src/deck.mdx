...
Finally, React comes with its own testing tools, which enzyme is a wrapper over
They do the job, but are a little less user-friendly
They don't rely on a virtual dom, so support react native
particularly good for snapshot tests
...
To guide us through looking at the options
I like having a concrete example to use in a talk,
...
its children, which are TodoItem components. But it doesn't render them, so it doesn't go all the way to the level of buttons and inputs
...
Can assert on any child component
