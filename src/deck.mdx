...
So instead of saying, "find an input with the right css class, and check its value prop", you can just say, 'yo, is there an input with the right text on the page'?

If you change the structure of the components, or switch from input to text area, this test will keep passing

And there are lots of nice selectors, you can query by label, or placeholder, or a special test data id. 
